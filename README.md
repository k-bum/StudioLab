...
- 주요 기능 소개  
여러 쇼핑몰 내 이미지와 텍스트 크롤링할 수 있는 크롤러 다중 생성  
쇼핑몰 별, 어트리뷰트(플레어스커트, 펜슬스커트 등) 별, 이미지 또는 텍스트 별 프로젝트 생성(예: farfetch몰의 플레어스커트 이미지 크롤링)  
프로젝트 단위로 크롤러 생성 관리 및 크롤링된 이미지 데이터 관리(각 크롤러는 비동기 방식으로 처리)  
크롤링된 이미지 및 텍스트 조회 및 삭제 기능  
batch program 구현(5초마다 각 크롤러들의 상태(run, stopped 등) 업데이트)   
크롤링된 이미지 자체 페이지 DB에 저장 및 mysql 연동  
크롤링된 이미지 어트리뷰트 기준 또는 모델 유무, 모델의 포즈 기준으로 필터링 후 .zip 파일로 다운로드  
최종 개발 완료된 페이지 배포(AWS)
